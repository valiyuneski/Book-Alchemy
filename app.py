...
def add_author():
    ...
    if request.method == 'POST':
        ...
        birth_date = request.form.get('birth_date').strip() # type: ignore
        death_date = request.form.get('death_date').strip() # type: ignore
        author = Author(name=author_name, birth_date=birth_date, death_date=death_date) # type: ignore
...
def add_book():
    ...
    if request.method == 'POST':
        ...
        author_id = request.form.get('author_id').strip() # type: ignore
        publication_year = request.form.get('publication_year').strip() # type: ignore
        isbn = request.form.get('isbn').strip() # type: ignore
        book = Book(title=title, author_id=author_id, publication_year=publication_year, isbn=isbn) # type: ignore
        db.session.add(book)
        db.session.commit()
